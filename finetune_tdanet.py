...
##### 随机初始化，从scrach训练
    ...
    if config["exp"]["exp_name"] == 'TDANet':
    ...
    elif config["exp"]["exp_name"] == 'TDANet_EchoSet':
        print('using TDANet_EchoSet')
        ...
        sys.exit()
    elif config["exp"]["exp_name"] == 'TDANet_LibriMix':
        print('using TDANet_LibriMix')
        model_path = '/public/ly/other/Look2hear/experiments/checkpoint/TDANet_LibriMix_ori/epoch=129_used.ckpt'
    ...
    for name, module in model.named_modules():
        ...
        if name == 'sm.unet.globalatt.mlp.fc1.conv':
            oriweight = model.state_dict()[name + '.weight']
            oriweight_lst.append(oriweight)
        elif name == 'sm.unet.globalatt.mlp.fc2.conv':
            oriweight = model.state_dict()[name + '.weight']
            oriweight_lst.append(oriweight)
        elif name == 'sm.unet.globalatt.mlp.dwconv':
            oriweight = model.state_dict()[name + '.weight']
            oriweight_lst.append(oriweight)
    ...
    for name, module in pruned_model.named_modules():
        ...
        if name == 'sm.unet.globalatt.mlp.fc1.conv':
            ...
            # print(mask_fc1.type)
            print(oriweight_lst[0][mask_fc1.bool().cpu()])
            pruned_state_dict[name + '.weight']=oriweight_lst[0][mask_fc1.bool().cpu()]
        elif name == 'sm.unet.globalatt.mlp.dwconv':
            # print(oriweight_lst[1].shape, pruned_model.state_dict()[name + '.weight'].shape)
            pruned_state_dict[name + '.weight']=oriweight_lst[1][mask_fc1.bool().cpu()]
        elif name == 'sm.unet.globalatt.mlp.fc2.conv':
            # print(oriweight_lst[2].shape, pruned_model.state_dict()[name + '.weight'].shape)
            pruned_state_dict[name + '.weight']=oriweight_lst[2][:,mask_fc1.bool().cpu(),:]

   
    pruned_model.load_state_dict(pruned_state_dict)
    print(pruned_model.state_dict()['sm.unet.globalatt.mlp.fc1.conv.weight'])
####################################################################################################################    
