...
from look2hear.models.sudormrf_pruned import ConvNormAct_pruned
...
##### 随机初始化，从scrach训练
    ...
    if config["exp"]["exp_name"] == 'SuDORMRF':
        print('using SuDORMRF')
        model_path = '/public/ly/other/Look2hear/look2hear/exp/tdanet/sudormrf_LRS2.pth'
    elif config["exp"]["exp_name"] == 'SuDORMRF_EchoSet':
        print('using SuDORMRF_EchoSet')
        model_path = '/public/ly/other/Look2hear/experiments/checkpoint/SuDORMRF_EchoSet_ori/epoch=319.ckpt'
    elif config["exp"]["exp_name"] == 'SuDORMRF_LibriMix':
        print('using SuDORMRF_LibriMix')
    ...
    for i in range(16):
        lst = []
        lst.append(model.state_dict()['sm.{}.proj_1x1.conv.weight'.format(i)])
        lst.append(model.state_dict()['sm.{}.spp_dw.0.conv.weight'.format(i)])
        lst.append(model.state_dict()['sm.{}.spp_dw.1.conv.weight'.format(i)])
        lst.append(model.state_dict()['sm.{}.spp_dw.2.conv.weight'.format(i)])
        lst.append(model.state_dict()['sm.{}.spp_dw.3.conv.weight'.format(i)])
        lst.append(model.state_dict()['sm.{}.spp_dw.4.conv.weight'.format(i)])
        oriweight_lst.append(lst)

    pruned_model = copy.deepcopy(model)
    for i in range(16):
        # print(i)
        pruned_model.sm[i] = UConvBlock(out_channels=128,
                       in_channels=num_list[i],
                       upsampling_depth=5)

    print(pruned_model.state_dict()['sm.0.proj_1x1.conv.weight'])
    print('pruned')
    pruned_state_dict = pruned_model.state_dict()
    for i in range(16):
        mask1 = mask[i]
        # print(oriweight_lst[i][0].shape, mask1.shape)
        print(pruned_state_dict['sm.{}.proj_1x1.conv.weight'.format(i)].shape, oriweight_lst[i][0][mask1.bool().cpu()].shape)
        print('111111111')
        pruned_state_dict['sm.{}.proj_1x1.conv.weight'.format(i)]=oriweight_lst[i][0][mask1.bool().cpu()]
        pruned_state_dict['sm.{}.spp_dw.0.conv.weight'.format(i)]=oriweight_lst[i][1][mask1.bool().cpu()]
        pruned_state_dict['sm.{}.spp_dw.1.conv.weight'.format(i)]=oriweight_lst[i][2][mask1.bool().cpu()]
        pruned_state_dict['sm.{}.spp_dw.2.conv.weight'.format(i)]=oriweight_lst[i][3][mask1.bool().cpu()]
        pruned_state_dict['sm.{}.spp_dw.3.conv.weight'.format(i)]=oriweight_lst[i][4][mask1.bool().cpu()]
        pruned_state_dict['sm.{}.spp_dw.4.conv.weight'.format(i)]=oriweight_lst[i][5][mask1.bool().cpu()]
