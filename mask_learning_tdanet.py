...
# from look2hear.models.tdanet_v2 import ConvNorm_pruned_in, ConvNorm_pruned_out
from look2hear.models.tdanet_v2 import ConvNorm_pruned, FFN
...
parser.add_argument(
    "--conf_dir",
    default="configs/mask_tdanet.yml",
...
# ##### 随机初始化，从scrach训练
    ...
    print(model)
#  ##### 加载预训练权重 retrain
    if config["exp"]["exp_name"] == 'MaskTDANet':
        print('using TDANet')
        model_path = '/public/ly/other/Look2hear/look2hear/exp/tdanet/tdanet_LRS2_bak_best_model.pth'
    elif config["exp"]["exp_name"] == 'MaskTDANet_EchoSet':
        print('using MaskTDANet_EchoSet')
        model_path = '/public/ly/other/Look2hear/experiments/checkpoint/TDANet_EchoSet_ori/epoch=280.ckpt'
    elif config["exp"]["exp_name"] == 'MaskTDANet_LibriMix':
        print('using MaskTDANet_LibriMix')
        model_path = '/public/ly/other/Look2hear/experiments/checkpoint/TDANet_LibriMix_ori/epoch=129_used.ckpt'
    elif config["exp"]["exp_name"] == 'MaskTDANet_Large':
        print('using TDANet_Large, not finished')
        sys.exit()

    # model_path = os.path.join('Experiments/checkpoint/TDANet', "best_model.pth") 
    # ori_model =  getattr(look2hear.models, 'TDANet').from_pretrain(
    #     model_path,
    ...
    for name, module in model.named_modules():
        if isinstance(module, ConvNorm_pruned) and module.enable_pruning:
            module.mask.alpha.requires_grad = True
        # if isinstance(module, FFN) and module.enable_pruning:
        #     module.mask.alpha.requires_grad = True
...
####################################################################################################################    
    ...
    for name, module in model.named_modules():
        if isinstance(module, ConvNorm_pruned) and module.enable_pruning:
            # print(module.get_sparsity())
            print(module.get_alpha())
            updated_weight.append(module.mask.alpha)
