...
##### 随机初始化，从scrach训练
    ...
    elif config["exp"]["exp_name"] == 'AFRCNN16':
        print('using AFRCNN16, not finished')
        sys.exit()
    elif config["exp"]["exp_name"] == 'AFRCNN12_LibriMix':
    ...
    # 示例用法
    # print(model)
    # mask = torch.load('/public/ly/other/Look2hear/save/afrcnn12_LRS2_mask.npy')  # LRS2 0.7
    # mask = torch.load('/public/ly/other/Look2hear/save/MASKAFRCNN12_EchoSet_mask.npy')  #  EchoSet 0.7
    # mask = torch.load('/public/ly/other/Look2hear/save/MASKAFRCNN12_mask_0.5.npy')  # LRS2 0.5
    # mask = torch.load('/public/ly/other/Look2hear/save/MASKAFRCNN12_mask_0.9.npy')  # LRS2 0.9
    # mask = torch.load('/public/ly/other/Look2hear/save/MASKAFRCNN12_mask_0.8.npy')  # LRS2 0.8
    mask = torch.load('/public/ly/other/Look2hear/save/MASKAFRCNN12_LibriMix_mask.npy')  # lribimix 0.7
    ...
    for name, module in model.named_modules():
        ...
        # print(name)
        if name == 'sm.blocks.concat_layer.0.conv':
            oriweight_lst.append(model.state_dict()[name + '.weight'])
        if name == 'sm.blocks.concat_layer.1.conv':
            oriweight_lst.append(model.state_dict()[name + '.weight'])
        if name == 'sm.blocks.concat_layer.2.conv':
            oriweight_lst.append(model.state_dict()[name + '.weight'])
        if name == 'sm.blocks.concat_layer.3.conv':
            oriweight_lst.append(model.state_dict()[name + '.weight'])
        if name == 'sm.blocks.last_layer.0.conv':
            oriweight_lst.append(model.state_dict()[name + '.weight'])

    pruned_model = copy.deepcopy(model)
    pruned_model.sm.blocks.concat_layer[0] = ConvNormAct(1024, num1, 1)
    pruned_model.sm.blocks.concat_layer[1] = ConvNormAct(1536, num2, 1)
    pruned_model.sm.blocks.concat_layer[2] = ConvNormAct(1536, num3, 1)
    pruned_model.sm.blocks.concat_layer[3] = ConvNormAct(1024, num4, 1)
    pruned_model.sm.blocks.last_layer[0] = ConvNormAct(num1+num2+num3+num4, 512, 1)
    print(pruned_model)
    # input_size = (1,16000)
    # calculate_flops(pruned_model, input_size)
    ...
    for name, module in pruned_model.named_modules():
        name = name.replace('module.', '')
        if name == 'sm.blocks.concat_layer.0.conv':
            # print(oriweight_lst[0].shape, pruned_model.state_dict()[name + '.weight'].shape)
            print(pruned_model.state_dict()[name + '.weight'])
            print(oriweight_lst[0][mask1.bool().cpu()])
            pruned_state_dict[name + '.weight']=oriweight_lst[0][mask1.bool().cpu()]
        elif name == 'sm.blocks.concat_layer.1.conv':
            # print(oriweight_lst[1].shape, pruned_model.state_dict()[name + '.weight'].shape)
            pruned_state_dict[name + '.weight']=oriweight_lst[1][mask2.bool().cpu()]
        elif name == 'sm.blocks.concat_layer.2.conv':
            # print(oriweight_lst[2].shape, pruned_model.state_dict()[name + '.weight'].shape)
            pruned_state_dict[name + '.weight']=oriweight_lst[2][mask3.bool().cpu()]
        elif name == 'sm.blocks.concat_layer.3.conv':
            # print(oriweight_lst[3].shape, pruned_model.state_dict()[name + '.weight'].shape)
            pruned_state_dict[name + '.weight']=oriweight_lst[3][mask4.bool().cpu()]
        elif name == 'sm.blocks.last_layer.0.conv':
            # print(oriweight_lst[4].shape, pruned_model.state_dict()[name + '.weight'].shape)
            pruned_state_dict[name + '.weight']=oriweight_lst[4][:,total_mask.bool().cpu(),:]
    
    pruned_model.load_state_dict(pruned_state_dict)
    print(pruned_model.state_dict()['sm.blocks.concat_layer.0.conv.weight'])
    input_size = (1,16000)
    calculate_flops(pruned_model, input_size)
####################################################################################################################    
    ...
    config["main_args"]["exp_dir"] = os.path.join(
        os.getcwd(), "experiments", "checkpoint", config["exp"]["exp_name"]+'_prune_0.8'
    )
